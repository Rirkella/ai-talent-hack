  ...
  return (
    <>
      ...
      <Section
        ...
        hint={
          <>
            Критерии, по которым студенты набирают меньше всего. Кандидаты на
            доработку материала курса. Средняя доля от максимума по каждому
            критерию, от худшего к лучшему. Именно доля, а не сырой балл: критерии
            весят по-разному, и на абсолютной шкале самый дорогой всегда выглядел
            бы худшим. Справа — доля цитат, которые код нашёл в работе: низкая
            означает не слабую работу, а слабое место самого механизма
            доказательств.
          </>
        }
        defaultOpen={false}
      ...
      <Section
        ...
        hint={
          <>
            Разбивка очереди: сколько работ автоматика проверила уверенно, а
            сколько отдала человеку. Сколько работ в каждой зоне. Границы те же,
            что в очереди ревьюера — «высоким» интерфейс и аналитика обязаны
      ...
      <Section
        ...
        hint={
          <>
            Как часто человек оставляет предварительный балл без правки. Низкое
            согласие — повод пересмотреть критерии. Важно не только сколько раз
            балл правили, но и на сколько: правка на полбалла и правка на четыре —
            разные истории. Каждая точка — одна подтверждённая работа, положение —
            насколько человек сдвинул предварительный балл.
          </>
        }
        defaultOpen={false}
      >
      ...
      <Section
        title="Сколько времени занимает проверка"
        hint={
          <>
      ...
      <Section
        ...
        hint={
          <>
            Когда студенты сдают работы. Пик у дедлайна — сигнал сдвинуть срок или
            напоминания. Дни идут подряд, включая пустые: пропуск дня без сдач
      ...
      <Section
        ...
        hint={
          <>
            Что чаще всего нарушают: общие для потока пробелы, которые стоит
            разобрать на занятии. Кластеризация пробелов со всех проверенных
            работ. Частая ошибка — сигнал о том, что тему стоит подробнее раскрыть
